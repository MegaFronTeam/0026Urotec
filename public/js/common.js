...
function eventHandler() {
	...
	let list = document.querySelectorAll('.sources li'); 
	
	document.addEventListener('click', function (event) {
		let target = event.target.closest('sup');
		if (!target) return;
		let arr = target.innerHTML.split(',');
		let index = +arr[0]; 
		for (let i = 0; i < list.length; i++) {  
			if (i == (index - 1)) { 
				function goToLi() {  
					let destination = $(list[i]).offset().top;
					$('html, body').animate({ scrollTop: destination - 80 }, 1000);
					Fancybox.close();
				}
				if ($(".sources ol").is(":hidden")) {
					$(".sources ol").slideDown(function () {
						goToLi();
					});
				}
				else {
